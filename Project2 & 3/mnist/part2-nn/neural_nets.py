...
def rectified_linear_unit(x):
    """ Returns the ReL
    U of x, or the maximum between 0 and x."""
    return max(0,x)
...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        activated_output = output_layer_activation_derivative(activated_output)

        ### Backpropagation ###
        output_layer_error = 0.5 * (y - activated_output) ** 2  # Scalar
        hidden_layer_error = np.multiply(
            np.dot(self.hidden_to_output_weights.T, (y - activated_output)),
            np.vectorize(rectified_linear_unit_derivative)(hidden_layer_weighted_input)
        )  # 3 by 1 matrix

        bias_gradients = hidden_layer_error
        hidden_to_output_weight_gradients = np.dot((y - activated_output), hidden_layer_activation.T)
